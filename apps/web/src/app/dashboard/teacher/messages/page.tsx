            ...
            <>
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.map((m: any, i: number) => (
                  <div key={m.id || i} className={`flex ${m.sender?.id === activeConv ? '' : 'justify-end'}`}>
                    <div className={`max-w-[70%] rounded-xl px-4 py-2 text-sm ${m.sender?.id === activeConv ? 'bg-slate-800 text-slate-200' : 'bg-emerald-600 text-white'}`}>
                      <p>{m.content}</p>
              ...
              <div className="border-t border-slate-800 p-4 flex gap-2">
                ...
                <button onClick={handleSend} disabled={sending || !content.trim()}
                  className="bg-emerald-600 hover:bg-emerald-500 text-white p-2.5 rounded-xl transition disabled:opacity-50">
